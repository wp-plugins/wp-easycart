	...
	{		
		...
		function getgiftcards($startrecord, $limit, $orderby, $ordertype, $filter) {
			  ...
			  $query= mysql_query("SELECT SQL_CALC_FOUND_ROWS 
				...
				ec_giftcard.* 
				FROM ec_giftcard LEFT JOIN ec_orderdetail on (ec_giftcard.giftcard_id = ec_orderdetail.giftcard_id) 
				WHERE ec_giftcard.giftcard_id != '' ".$filter." ORDER BY ".  $orderby ." ".  $ordertype . " LIMIT ".  $startrecord .", ".  $limit."");
